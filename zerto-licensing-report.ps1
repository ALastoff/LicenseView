...
try {
    ...
    # Import Zerto.Auth module (Keycloak + legacy support)
    $zertoAuthPath = Join-Path $ScriptRoot "src\ps\Zerto.Auth.psm1"
    if (-not (Test-Path $zertoAuthPath)) {
        Write-Host "[ERROR] Auth module not found at: $zertoAuthPath" -ForegroundColor Red
        exit 3
    }
    Write-Host "[INFO] Loading Zerto authentication module..." -ForegroundColor Cyan
...
function Send-UsageReport {
    ...
    $auth = Connect-ZertoApi -ZvmUrl $zvmUrl -Username $username -Password $password -VerifyTls $verifyTls -TimeoutSeconds $configData.timeout_seconds
